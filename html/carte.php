<!doctype html>
<html lang="fr">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="wclassth=device-wclassth,initial-scale=1">
    <link rel="stylesheet" href="./css/style.css">
    <title>Pizzeria de la plage</title>
</head>

<body>
    <header>
        <nav class="menus1">
            <label class="tel"><a href="tel:+33600000000"> 0600000000</a></label>
            <ul>

                <li><a href="#">Panier</a></li>
                <li><a href="./carte.php">Carte</a></li>
                <li><a href="./index.php">Accueil</a></li>

            </ul>
        </nav>
        <nav class="menus2">
            <label class="tel"><a href="tel:+33600000000">✆</a></label>
            <ul class="menuDeroulant">
                <li class="menuBurger">☰</li>
                <li><a href="./index.php">Accueil</a></li>
                <li><a href="./carte.php">Carte</a></li>
                <li><a href="#">Panier</a></li>


            </ul>

        </nav>
        <img class="pizza" src="./img/pizza.png" alt="logo">
    </header>
    <main>
        <section class="menu">
            <div class="container">
                <div class="menu-heading">Nos Pizzas</div>
                <div class="menu-grid">
                    <div class="menu-hold">
                        <div class="menu-container">
                            <div class="menu-img">
                                &nbsp;

                            </div>
                            <h1 class="menu-title">L'orientale</h1>
                            <p class="menu-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus doloribus, ea veniam velit sed harum qui perferendis debitis cumque obcaecati.</p>
                            <aside class="prix">
                                <p class="tarifs">La Part <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class=" tarifs">Moyenne <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class="tarifs">Grande <input type="number" step="1" name="nombre" value="0" /></p>
                            </aside>
                            <h3 class="price">10.99€</h3>
                        </div>
                    </div>
                    <div class="menu-hold">
                        <div class="menu-container">
                            <div class="menu-img">
                                &nbsp;
                            </div>
                            <h1 class="menu-title">La saumon</h1>
                            <p class="menu-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus doloribus, ea veniam velit sed harum qui perferendis debitis cumque obcaecati.</p>
                            <aside class="prix">
                                <p class="tarifs">La Part <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class=" tarifs">Moyenne <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class="tarifs">Grande <input type="number" step="1" name="nombre" value="0" /></p>
                            </aside>
                            <h3 class="price">10.99€</h3>
                        </div>
                    </div>
                    <div class="menu-hold">
                        <div class="menu-container">
                            <div class="menu-img">
                                &nbsp;
                            </div>
                            <h1 class="menu-title">La Savoyarde</h1>
                            <p class="menu-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus doloribus, ea veniam velit sed harum qui perferendis debitis cumque obcaecati.</p>
                            <aside class="prix">
                                <p class="tarifs">La Part <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class=" tarifs">Moyenne <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class="tarifs">Grande <input type="number" step="1" name="nombre" value="0" /></p>
                            </aside>
                            <h3 class="price">10.99€</h3>
                        </div>
                    </div>
                    <div class="menu-hold">
                        <div class="menu-container">
                            <div class="menu-img">
                                &nbsp;
                            </div>
                            <h1 class="menu-title">La Campagnarde</h1>
                            <p class="menu-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus doloribus, ea veniam velit sed harum qui perferendis debitis cumque obcaecati.</p>
                            <aside class="prix">
                                <p class="tarifs">La Part <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class=" tarifs">Moyenne <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class="tarifs">Grande <input type="number" step="1" name="nombre" value="0" /></p>
                            </aside>
                            <h3 class="price">10.99€</h3>
                        </div>
                    </div>
                    <div class="menu-hold">
                        <div class="menu-container">
                            <div class="menu-img">
                                &nbsp;
                            </div>
                            <h1 class="menu-title">La Greques</h1>
                            <p class="menu-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus doloribus, ea veniam velit sed harum qui perferendis debitis cumque obcaecati.</p>
                            <aside class="prix">
                                <p class="tarifs">La Part <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class=" tarifs">Moyenne <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class="tarifs">Grande <input type="number" step="1" name="nombre" value="0" /></p>
                            </aside>
                            <h3 class="price">10.99€</h3>
                        </div>
                    </div>
                    <div class="menu-hold">
                        <div class="menu-container">
                            <div class="menu-img">
                                &nbsp;
                            </div>
                            <h1 class="menu-title">La fruits de mers</h1>
                            <p class="menu-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus doloribus, ea veniam velit sed harum qui perferendis debitis cumque obcaecati.</p>
                            <aside class="prix">
                                <p class="tarifs">La Part <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class=" tarifs">Moyenne <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class="tarifs">Grande <input type="number" step="1" name="nombre" value="0" /></p>
                            </aside>
                            <h3 class="price">10.99€</h3>
                        </div>
                    </div>
                    <div class="menu-hold">
                        <div class="menu-container">
                            <div class="menu-img">
                                &nbsp;
                            </div>
                            <h1 class="menu-title">L'authentique</h1>
                            <p class="menu-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus doloribus, ea veniam velit sed harum qui perferendis debitis cumque obcaecati.</p>
                            <aside class="prix">
                                <p class="tarifs">La Part <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class=" tarifs">Moyenne <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class="tarifs">Grande <input type="number" step="1" name="nombre" value="0" /></p>
                            </aside>
                            <h3 class="price">10.99€</h3>
                        </div>
                    </div>
                    <div class="menu-hold">
                        <div class="menu-container">
                            <div class="menu-img">
                                &nbsp;
                            </div>
                            <h1 class="menu-title">La Deluxe</h1>
                            <p class="menu-p">Lorem ipsum dolor sit amet consectetur adipisicing elit. Temporibus doloribus, ea veniam velit sed harum qui perferendis debitis cumque obcaecati.</p>
                            <aside class="prix">
                                <p class="tarifs">La Part <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class=" tarifs">Moyenne <input type="number" step="1" name="nombre" value="0" /></p>
                                <p class="tarifs">Grande <input type="number" step="1" name="nombre" value="0" /></p>
                            </aside>
                            <h3 class="price">10.99€</h3>
                        </div>
                    </div>
                </div>
            </div>
            <div style="height: 100px;">&nbsp;</div>
        </section>
    </main>
    <footer>
        <div class="icons"><a href="https://www.google.fr/maps/place/2+Av.+de+Montredon,+13008+Marseille/@43.2453546,5.372393,19z/data=!4m8!1m2!2m1!1s1+plage+du+prado!3m4!1s0x12c9c70b3322b68b:0x3aa8471c8973edd2!8m2!3d43.2453536!4d5.3729402">⚲</a><a href="mailto:pizzeriadelaplage@gmail.com">✉</a></div>
    </footer>
</body>

</html>